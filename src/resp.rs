...
fn decode_array(buffer: BytesMut) -> Result<Option<(Value, usize)>> {
    // assigning the array_length and bytes_consumed to the result of the if let Some((line, len)) statement

    let (array_length, mut bytes_consumed) =
         // if let Some((line, len)) is for checking if the buffer contains a line and a length (the line is the array length, and the length is the length of the array)
         // read_until_crlf takes the buffer and returns a Result of an Option (the Option is Some if the buffer contains a line and a length, and None if it does not)
            if let Some((line, len)) = read_until_crlf(&buffer[1..]) {
                // if the buffer contains a line and a length, parse the integer from the line
                let array_length = parse_integer(line)?;
                // return the array_length and the length of the array
                (array_length, len + 1)
            } else {
              // else return Ok(None), which means the buffer does not contain a line and a length
                return Ok(None);
            };
